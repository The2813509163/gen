...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        #my_modify
        self.update_step = int(os.getenv("update_step",0))
        tap_args = os.environ.get("tap_args")
        import json
        self.tap_args = json.loads(tap_args) if tap_args else None
        setattr(self.mlp, "update_step", self.update_step)
        setattr(self.mlp, "tap_args", self.tap_args)#my_modify
        # TAP
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.tap_args.get("tap_enabled", False):
            ...
            setattr(self.mlp, "tap_stop_at_steps", tap_stop_at_steps)#my_modify
        ...
        if (self.model.learnable_mask.lower() == "true") and self.model.layers[0].update_step > 0.02*self.model.layers[0].tap_args["tap_stop_at_steps"]:
            ...
            if self.model.layers[0].update_step % 10 == 0:
                try:
                    ...
                    print(f" pruning_loss: {pruning_loss:.3f}, gt0: {(self.model.HSM_mask_proxy>0.0).sum()}/{self.model.HSM_mask_proxy.shape[-1]}, pruning_l2_loss: {pruning_l2_loss:.3f}, pruning_l1_loss: {pruning_l1_loss:.3f}, pruning_interm_loss: {pruning_interm_loss:.3f}")
