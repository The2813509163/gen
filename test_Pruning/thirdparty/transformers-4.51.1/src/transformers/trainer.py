...
from transformers.dummy import GaussianStratifiedSampler,ZipfSampler,PretrainDataSampler,MixedDistributionSampler
...
class CustomTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            teacher_model (nn.Module): 冻结的、用作教师的原始模型。
            alpha (float): 平衡微调损失和蒸馏损失的权重。范围 [0, 1]。
            temperature (float): 知识蒸馏中用于软化logits的温度。
            *args, **kwargs: 传递给基类Trainer的常规参数。
        """
        ...
        # print("FFFFFFFFFFFFFFFFFLASH")
        # print(self.teacher_model)
        self.alpha = alpha
        self.temperature = temperature
        
        # 确保教师模型在正确的设备上，并处于评估模式
    ...
    def compute_loss(self, model, inputs, return_outputs=False,num_items_in_batch=None):
        """
        重写compute_loss方法。
        'inputs'只包含微调数据。dummy数据将通过调用生成器函数获得。
        """
        ...
        # 4. 生成一个批次的dummy数据
        dummy_inputs_ids = self.sampler.generate_batch(
            batch_size=2, 
            seq_len=target_sequence_length
            # strata_weights=custom_weights
        )
        dummy_inputs = {
            "input_ids": dummy_inputs_ids.to(self.args.device),
            "attention_mask": torch.ones_like(dummy_inputs_ids,device=self.args.device)
        }
        # --- 接下来的逻辑和之前一样 ---
        with torch.no_grad():
            outputs_teacher_dummy = self.teacher_model(**dummy_inputs)
        ...
        pruning_loss,outputs = super().compute_loss(model, combined_inputs, return_outputs=True,num_items_in_batch=None)

        logits_finetune = outputs.logits[:finetune_batch_size]
        logits_student = outputs.logits[finetune_batch_size:]
        ...
        # 如果正在进行评估，compute_loss可能在一个不同的上下文中被调用
        # 我们可以通过检查模型是否在训练模式来安全地调用self.log
        if self.is_in_train:
            self.log(log_data)
...
class DummyTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        freq_file_path = "/data/home/duyuanProfV2/workspace/qianxuzhen/data/kris/qianxuzhen/dummy_info/pretrain_data_frequency.pkl"
        math_freq_path = "/data/home/duyuanProfV2/workspace/qianxuzhen/data/kris/qianxuzhen/dummy_info/math_frequency.pkl"    
        # self.sampler = MixedDistributionSampler(tokenizer=tokenizer,pretrain_freq_path=freq_file_path,math_freq_path=math_freq_path)
        # 初始化损失函数
        self.sampler = PretrainDataSampler(tokenizer=tokenizer,freq_file_path=freq_file_path)
        self.loss_fn_distill = nn.KLDivLoss(reduction='batchmean')
...
# 假设您的模型类和依赖已导入
# from your_model_file import BaseLlamaForCausalLM 
...
class SuperTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 创建一个可以无限循环提供数据的生成器
        def _infinite_loader(dataset):
            while True:
                for sample in dataset:
                    yield sample
        
        self.dummy_iterator = _infinite_loader(self.dummy_dataset)
